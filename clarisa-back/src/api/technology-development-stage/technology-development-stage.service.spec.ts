  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: TechnologyDevelopmentStageRepository, useValue: mockTechnologyDevelopmentStageRepository },
      ],
    }).compile();
  ...
  it('should be defined', () => {
    ...
    it('should return items on findAll with SHOW_ALL', async () => {
      ...
      Object.keys(mockTechnologyDevelopmentStageRepository).forEach(k => {
        if (typeof mockTechnologyDevelopmentStageRepository[k]?.mockResolvedValue === 'function') {
          mockTechnologyDevelopmentStageRepository[k].mockResolvedValue(mockItems);
        }
      });
    ...
    it('should return a single item on findOne', async () => {
      const mockItem = { id: 1 };
      mockTechnologyDevelopmentStageRepository.findOneBy = mockTechnologyDevelopmentStageRepository.findOneBy || jest.fn();
      mockTechnologyDevelopmentStageRepository.findOne = mockTechnologyDevelopmentStageRepository.findOne || jest.fn();
      mockTechnologyDevelopmentStageRepository.findOneBy.mockResolvedValue(mockItem);
      mockTechnologyDevelopmentStageRepository.findOne.mockResolvedValue(mockItem);
    ...
    it('should save items on update', async () => {
      ...
      expect(mockTechnologyDevelopmentStageRepository.save).toHaveBeenCalledWith(dto);
    });
});
