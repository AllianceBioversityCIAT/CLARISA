...
describe('InnovationCharacteristicService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: InnovationCharacteristicRepository,
          useValue: mockInnovationCharacteristicRepository,
        },
      ],
    }).compile();
  ...
  it('should return items on findAll with SHOW_ALL', async () => {
    ...
    Object.keys(mockInnovationCharacteristicRepository).forEach((k) => {
      if (
        typeof mockInnovationCharacteristicRepository[k]?.mockResolvedValue ===
        'function'
      ) {
  ...
  it('should return a single item on findOne', async () => {
    const mockItem = { id: 1 };
    mockInnovationCharacteristicRepository.findOneBy =
      mockInnovationCharacteristicRepository.findOneBy || jest.fn();
    mockInnovationCharacteristicRepository.findOne =
      mockInnovationCharacteristicRepository.findOne || jest.fn();
    mockInnovationCharacteristicRepository.findOneBy.mockResolvedValue(
      mockItem,
    );
