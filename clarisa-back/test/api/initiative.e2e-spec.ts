...
describe('Initiatives (e2e)', () => {
  ...
  it('/api/initiatives/get/1 (GET)', () => {
    return request(app.getHttpServer())
      .get(`/api/initiatives/get/1`)
      .expect(HttpStatus.OK)
      .expect((res) => {
        const data = res.body;
        ...
        expect(data).toHaveProperty('official_code');
      });
  });

  //The endpoint is used to search by id, an invalid id is sent to it.
  it('/api/initiatives/get/a (GET)', () => {
    return request(app.getHttpServer())
      .get('/api/initiatives/get/' + 'a')
      .expect(400);
  });
