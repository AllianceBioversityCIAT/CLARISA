...
describe('TechnicalFieldService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: TechnicalFieldRepository,
          useValue: mockTechnicalFieldRepository,
        },
      ],
    }).compile();
  ...
  it('should return items on findAll with SHOW_ALL', async () => {
    ...
    Object.keys(mockTechnicalFieldRepository).forEach((k) => {
      if (
        typeof mockTechnicalFieldRepository[k]?.mockResolvedValue === 'function'
      ) {
  ...
  it('should return a single item on findOne', async () => {
    const mockItem = { id: 1 };
    mockTechnicalFieldRepository.findOneBy =
      mockTechnicalFieldRepository.findOneBy || jest.fn();
    mockTechnicalFieldRepository.findOne =
      mockTechnicalFieldRepository.findOne || jest.fn();
