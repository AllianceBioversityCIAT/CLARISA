...
describe('InvestmentTypeService', () => {
  ...
  const mockInvestmentTypeRepository: any = {
    find: jest.fn(),
    findOneBy: jest.fn(),
    save: jest.fn(),
    findOne: jest.fn(),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: InvestmentTypeRepository, useValue: mockInvestmentTypeRepository },
      ],
    }).compile();
  ...
  it('should be defined', () => {
    ...
    it('should return items on findAll with SHOW_ALL', async () => {
      ...
      Object.keys(mockInvestmentTypeRepository).forEach(k => {
        if (typeof mockInvestmentTypeRepository[k]?.mockResolvedValue === 'function') {
          mockInvestmentTypeRepository[k].mockResolvedValue(mockItems);
        }
      });
    ...
    it('should return a single item on findOne', async () => {
      const mockItem = { id: 1 };
      mockInvestmentTypeRepository.findOneBy = mockInvestmentTypeRepository.findOneBy || jest.fn();
      mockInvestmentTypeRepository.findOne = mockInvestmentTypeRepository.findOne || jest.fn();
      mockInvestmentTypeRepository.findOneBy.mockResolvedValue(mockItem);
      mockInvestmentTypeRepository.findOne.mockResolvedValue(mockItem);
    ...
    it('should save items on update', async () => {
      const dto = [{ id: 1 }];
      mockInvestmentTypeRepository.save.mockResolvedValue(dto);

      const result = await service.update(dto as any);
      expect(mockInvestmentTypeRepository.save).toHaveBeenCalledWith(dto);
    });
});
