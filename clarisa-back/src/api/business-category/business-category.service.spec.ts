...
describe('BusinessCategoryService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: BusinessCategoryRepository,
  ...
  it('should return items on findAll with SHOW_ALL', async () => {
    ...
    Object.keys(mockBusinessCategoryRepository).forEach((k) => {
      if (
        typeof mockBusinessCategoryRepository[k]?.mockResolvedValue ===
        'function'
      ) {
  ...
  it('should return a single item on findOne', async () => {
    const mockItem = { id: 1 };
    mockBusinessCategoryRepository.findOneBy =
      mockBusinessCategoryRepository.findOneBy || jest.fn();
    mockBusinessCategoryRepository.findOne =
      mockBusinessCategoryRepository.findOne || jest.fn();
