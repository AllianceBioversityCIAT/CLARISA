...
describe('PolicyTypeService', () => {
  ...
  it('should return items on findAll with SHOW_ALL', async () => {
    ...
    Object.keys(mockPolicyTypeRepository).forEach((k) => {
      if (
        typeof mockPolicyTypeRepository[k]?.mockResolvedValue === 'function'
      ) {
  ...
  it('should return a single item on findOne', async () => {
    const mockItem = { id: 1 };
    mockPolicyTypeRepository.findOneBy =
      mockPolicyTypeRepository.findOneBy || jest.fn();
    mockPolicyTypeRepository.findOne =
      mockPolicyTypeRepository.findOne || jest.fn();
