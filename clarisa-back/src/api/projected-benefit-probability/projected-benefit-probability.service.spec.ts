  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: ProjectedBenefitProbabilityRepository, useValue: mockProjectedBenefitProbabilityRepository },
      ],
    }).compile();
  ...
  it('should be defined', () => {
    ...
    it('should return items on findAll with SHOW_ALL', async () => {
      ...
      Object.keys(mockProjectedBenefitProbabilityRepository).forEach(k => {
        if (typeof mockProjectedBenefitProbabilityRepository[k]?.mockResolvedValue === 'function') {
          mockProjectedBenefitProbabilityRepository[k].mockResolvedValue(mockItems);
        }
      });
    ...
    it('should return a single item on findOne', async () => {
      const mockItem = { id: 1 };
      mockProjectedBenefitProbabilityRepository.findOneBy = mockProjectedBenefitProbabilityRepository.findOneBy || jest.fn();
      mockProjectedBenefitProbabilityRepository.findOne = mockProjectedBenefitProbabilityRepository.findOne || jest.fn();
      mockProjectedBenefitProbabilityRepository.findOneBy.mockResolvedValue(mockItem);
      mockProjectedBenefitProbabilityRepository.findOne.mockResolvedValue(mockItem);
    ...
    it('should save items on update', async () => {
      ...
      expect(mockProjectedBenefitProbabilityRepository.save).toHaveBeenCalledWith(dto);
    });
});
