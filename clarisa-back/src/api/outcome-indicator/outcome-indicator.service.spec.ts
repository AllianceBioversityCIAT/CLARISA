...
describe('OutcomeIndicatorService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: OutcomeIndicatorRepository,
          useValue: mockOutcomeIndicatorRepository,
        },
      ],
    }).compile();
  ...
  it('should return items on findAll with SHOW_ALL', async () => {
    ...
    Object.keys(mockOutcomeIndicatorRepository).forEach((k) => {
      if (
        typeof mockOutcomeIndicatorRepository[k]?.mockResolvedValue ===
        'function'
      ) {
  ...
  it('should return a single item on findOne', async () => {
    const mockItem = { id: 1 };
    mockOutcomeIndicatorRepository.findOneBy =
      mockOutcomeIndicatorRepository.findOneBy || jest.fn();
    mockOutcomeIndicatorRepository.findOne =
      mockOutcomeIndicatorRepository.findOne || jest.fn();
