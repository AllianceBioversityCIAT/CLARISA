...
describe('ProjectedBenefitDepthService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: ProjectedBenefitDepthRepository,
          useValue: mockProjectedBenefitDepthRepository,
        },
      ],
    }).compile();
  ...
  it('should return items on findAll with SHOW_ALL', async () => {
    ...
    Object.keys(mockProjectedBenefitDepthRepository).forEach((k) => {
      if (
        typeof mockProjectedBenefitDepthRepository[k]?.mockResolvedValue ===
        'function'
      ) {
  ...
  it('should return a single item on findOne', async () => {
    const mockItem = { id: 1 };
    mockProjectedBenefitDepthRepository.findOneBy =
      mockProjectedBenefitDepthRepository.findOneBy || jest.fn();
    mockProjectedBenefitDepthRepository.findOne =
      mockProjectedBenefitDepthRepository.findOne || jest.fn();
