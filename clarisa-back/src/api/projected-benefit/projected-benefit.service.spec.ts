  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: ProjectedBenefitRepository, useValue: mockProjectedBenefitRepository },
      ],
    }).compile();
  ...
  it('should be defined', () => {
    ...
    it('should return items on findAll with SHOW_ALL', async () => {
      ...
      Object.keys(mockProjectedBenefitRepository).forEach(k => {
        if (typeof mockProjectedBenefitRepository[k]?.mockResolvedValue === 'function') {
    ...
    it('should return a single item on findOne', async () => {
      const mockItem = { id: 1 };
      mockProjectedBenefitRepository.findOneBy = mockProjectedBenefitRepository.findOneBy || jest.fn();
      mockProjectedBenefitRepository.findOne = mockProjectedBenefitRepository.findOne || jest.fn();
