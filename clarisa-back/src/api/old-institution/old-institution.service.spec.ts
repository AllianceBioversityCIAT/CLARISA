...
describe('OldInstitutionService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: OldInstitutionRepository,
          useValue: mockOldInstitutionRepository,
        },
      ],
    }).compile();
  ...
  it('should return items on findAll with SHOW_ALL', async () => {
    ...
    Object.keys(mockOldInstitutionRepository).forEach((k) => {
      if (
        typeof mockOldInstitutionRepository[k]?.mockResolvedValue === 'function'
      ) {
  ...
  it('should return a single item on findOne', async () => {
    const mockItem = { id: 1 };
    mockOldInstitutionRepository.findOneBy =
      mockOldInstitutionRepository.findOneBy || jest.fn();
    mockOldInstitutionRepository.findOne =
      mockOldInstitutionRepository.findOne || jest.fn();
