...
describe('PolicyStageService', () => {
  ...
  const mockPolicyStageRepository: any = {
    find: jest.fn(),
    findOneBy: jest.fn(),
    save: jest.fn(),
    findOne: jest.fn(),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PolicyStageService,
        { provide: PolicyStageRepository, useValue: mockPolicyStageRepository },
      ],
    }).compile();
  ...
  it('should be defined', () => {
    ...
    it('should return items on findAll with SHOW_ALL', async () => {
      ...
      Object.keys(mockPolicyStageRepository).forEach(k => {
        if (typeof mockPolicyStageRepository[k]?.mockResolvedValue === 'function') {
          mockPolicyStageRepository[k].mockResolvedValue(mockItems);
        }
      });
    ...
    it('should return a single item on findOne', async () => {
      const mockItem = { id: 1 };
      mockPolicyStageRepository.findOneBy = mockPolicyStageRepository.findOneBy || jest.fn();
      mockPolicyStageRepository.findOne = mockPolicyStageRepository.findOne || jest.fn();
      mockPolicyStageRepository.findOneBy.mockResolvedValue(mockItem);
      mockPolicyStageRepository.findOne.mockResolvedValue(mockItem);
    ...
    it('should save items on update', async () => {
      const dto = [{ id: 1 }];
      mockPolicyStageRepository.save.mockResolvedValue(dto);

      const result = await service.update(dto as any);
      expect(mockPolicyStageRepository.save).toHaveBeenCalledWith(dto);
    });
});
