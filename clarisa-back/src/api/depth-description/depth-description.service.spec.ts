...
describe('DepthDescriptionService', () => {
  ...
  const mockDepthDescriptionRepository: any = {
    find: jest.fn(),
    findOneBy: jest.fn(),
    save: jest.fn(),
    findOne: jest.fn(),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: DepthDescriptionRepository, useValue: mockDepthDescriptionRepository },
      ],
    }).compile();
  ...
  it('should be defined', () => {
    ...
    it('should return items on findAll with SHOW_ALL', async () => {
      ...
      Object.keys(mockDepthDescriptionRepository).forEach(k => {
        if (typeof mockDepthDescriptionRepository[k]?.mockResolvedValue === 'function') {
          mockDepthDescriptionRepository[k].mockResolvedValue(mockItems);
        }
      });
    ...
    it('should return a single item on findOne', async () => {
      const mockItem = { id: 1 };
      mockDepthDescriptionRepository.findOneBy = mockDepthDescriptionRepository.findOneBy || jest.fn();
      mockDepthDescriptionRepository.findOne = mockDepthDescriptionRepository.findOne || jest.fn();
      mockDepthDescriptionRepository.findOneBy.mockResolvedValue(mockItem);
      mockDepthDescriptionRepository.findOne.mockResolvedValue(mockItem);
    ...
    it('should save items on update', async () => {
      const dto = [{ id: 1 }];
      mockDepthDescriptionRepository.save.mockResolvedValue(dto);

      const result = await service.update(dto as any);
      expect(mockDepthDescriptionRepository.save).toHaveBeenCalledWith(dto);
    });
});
