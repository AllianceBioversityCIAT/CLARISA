...
describe('InvestmentTypeService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: InvestmentTypeRepository,
          useValue: mockInvestmentTypeRepository,
        },
      ],
    }).compile();
  ...
  it('should return items on findAll with SHOW_ALL', async () => {
    ...
    Object.keys(mockInvestmentTypeRepository).forEach((k) => {
      if (
        typeof mockInvestmentTypeRepository[k]?.mockResolvedValue === 'function'
      ) {
  ...
  it('should return a single item on findOne', async () => {
    const mockItem = { id: 1 };
    mockInvestmentTypeRepository.findOneBy =
      mockInvestmentTypeRepository.findOneBy || jest.fn();
    mockInvestmentTypeRepository.findOne =
      mockInvestmentTypeRepository.findOne || jest.fn();
