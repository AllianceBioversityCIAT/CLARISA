...
describe('GeographicScopeService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: GeographicScopeRepository,
          useValue: mockGeographicScopeRepository,
        },
      ],
    }).compile();
  ...
  it('should return items on findAll with SHOW_ALL', async () => {
    ...
    Object.keys(mockGeographicScopeRepository).forEach((k) => {
      if (
        typeof mockGeographicScopeRepository[k]?.mockResolvedValue ===
        'function'
      ) {
  ...
  it('should return a single item on findOne', async () => {
    const mockItem = { id: 1 };
    mockGeographicScopeRepository.findOneBy =
      mockGeographicScopeRepository.findOneBy || jest.fn();
    mockGeographicScopeRepository.findOne =
      mockGeographicScopeRepository.findOne || jest.fn();
