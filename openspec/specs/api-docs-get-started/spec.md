...
The "Get Started" / authentication onboarding section of the public API documentation page (`clarisa-front/src/assets/api-reference/index.html`) — its sidebar entry, its content (token request flow, `X-API-Key` header usage, read-only notice), and how it integrates with the existing docs navigation.
