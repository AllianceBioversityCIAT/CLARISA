...
describe('GlobalTargetService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        GlobalTargetService,
        { provide: GlobalTargetRepository, useValue: mockGlobalTargetRepository },
      ],
    }).compile();
  ...
  it('should be defined', () => {
    ...
    it('should return items on findAll with SHOW_ALL', async () => {
      ...
      Object.keys(mockGlobalTargetRepository).forEach(k => {
        if (typeof mockGlobalTargetRepository[k]?.mockResolvedValue === 'function') {
    ...
    it('should return a single item on findOne', async () => {
      const mockItem = { id: 1 };
      mockGlobalTargetRepository.findOneBy = mockGlobalTargetRepository.findOneBy || jest.fn();
      mockGlobalTargetRepository.findOne = mockGlobalTargetRepository.findOne || jest.fn();
