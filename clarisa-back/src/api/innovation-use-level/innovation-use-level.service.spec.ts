...
describe('InnovationUseLevelService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: InnovationUseLevelRepository,
          useValue: mockInnovationUseLevelRepository,
        },
      ],
    }).compile();
  ...
  it('should return items on findAll with SHOW_ALL', async () => {
    ...
    Object.keys(mockInnovationUseLevelRepository).forEach((k) => {
      if (
        typeof mockInnovationUseLevelRepository[k]?.mockResolvedValue ===
        'function'
      ) {
  ...
  it('should return a single item on findOne', async () => {
    const mockItem = { id: 1 };
    mockInnovationUseLevelRepository.findOneBy =
      mockInnovationUseLevelRepository.findOneBy || jest.fn();
    mockInnovationUseLevelRepository.findOne =
      mockInnovationUseLevelRepository.findOne || jest.fn();
