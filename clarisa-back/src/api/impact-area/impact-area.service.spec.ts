...
describe('ImpactAreaService', () => {
  ...
  it('should return items on findAll with SHOW_ALL', async () => {
    ...
    Object.keys(mockImpactAreaRepository).forEach((k) => {
      if (
        typeof mockImpactAreaRepository[k]?.mockResolvedValue === 'function'
      ) {
  ...
  it('should return a single item on findOne', async () => {
    const mockItem = { id: 1 };
    mockImpactAreaRepository.findOneBy =
      mockImpactAreaRepository.findOneBy || jest.fn();
    mockImpactAreaRepository.findOne =
      mockImpactAreaRepository.findOne || jest.fn();
