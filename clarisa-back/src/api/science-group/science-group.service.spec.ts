...
describe('ScienceGroupService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ScienceGroupService,
        { provide: ScienceGroupRepository, useValue: mockScienceGroupRepository },
      ],
    }).compile();
  ...
  it('should be defined', () => {
    ...
    it('should return items on findAll with SHOW_ALL', async () => {
      ...
      Object.keys(mockScienceGroupRepository).forEach(k => {
        if (typeof mockScienceGroupRepository[k]?.mockResolvedValue === 'function') {
    ...
    it('should return a single item on findOne', async () => {
      const mockItem = { id: 1 };
      mockScienceGroupRepository.findOneBy = mockScienceGroupRepository.findOneBy || jest.fn();
      mockScienceGroupRepository.findOne = mockScienceGroupRepository.findOne || jest.fn();
