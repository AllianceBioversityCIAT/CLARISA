...
describe('RegionTypeService', () => {
  ...
  const mockRegionTypeRepository: any = {
    find: jest.fn(),
    findOneBy: jest.fn(),
    save: jest.fn(),
    findOne: jest.fn(),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        RegionTypeService,
        { provide: RegionTypeRepository, useValue: mockRegionTypeRepository },
      ],
    }).compile();
  ...
  it('should be defined', () => {
    ...
    it('should return items on findAll with SHOW_ALL', async () => {
      ...
      Object.keys(mockRegionTypeRepository).forEach(k => {
        if (typeof mockRegionTypeRepository[k]?.mockResolvedValue === 'function') {
          mockRegionTypeRepository[k].mockResolvedValue(mockItems);
        }
      });
    ...
    it('should return a single item on findOne', async () => {
      const mockItem = { id: 1 };
      mockRegionTypeRepository.findOneBy = mockRegionTypeRepository.findOneBy || jest.fn();
      mockRegionTypeRepository.findOne = mockRegionTypeRepository.findOne || jest.fn();
      mockRegionTypeRepository.findOneBy.mockResolvedValue(mockItem);
      mockRegionTypeRepository.findOne.mockResolvedValue(mockItem);
    ...
    it('should save items on update', async () => {
      const dto = [{ id: 1 }];
      mockRegionTypeRepository.save.mockResolvedValue(dto);

      const result = await service.update(dto as any);
      expect(mockRegionTypeRepository.save).toHaveBeenCalledWith(dto);
    });
});
