  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: ImpactAreaIndicatorRepository, useValue: mockImpactAreaIndicatorRepository },
      ],
    }).compile();

    service = module.get<ImpactAreaIndicatorService>(ImpactAreaIndicatorService);
  });
  ...
  it('should be defined', () => {
    ...
    it('should return items on findAll with SHOW_ALL', async () => {
      ...
      Object.keys(mockImpactAreaIndicatorRepository).forEach(k => {
        if (typeof mockImpactAreaIndicatorRepository[k]?.mockResolvedValue === 'function') {
    ...
    it('should return a single item on findOne', async () => {
      const mockItem = { id: 1 };
      mockImpactAreaIndicatorRepository.findOneBy = mockImpactAreaIndicatorRepository.findOneBy || jest.fn();
      mockImpactAreaIndicatorRepository.findOne = mockImpactAreaIndicatorRepository.findOne || jest.fn();
