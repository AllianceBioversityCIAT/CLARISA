...
describe('DepthDescriptionService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: DepthDescriptionRepository,
          useValue: mockDepthDescriptionRepository,
        },
      ],
    }).compile();
  ...
  it('should return items on findAll with SHOW_ALL', async () => {
    ...
    Object.keys(mockDepthDescriptionRepository).forEach((k) => {
      if (
        typeof mockDepthDescriptionRepository[k]?.mockResolvedValue ===
        'function'
      ) {
  ...
  it('should return a single item on findOne', async () => {
    const mockItem = { id: 1 };
    mockDepthDescriptionRepository.findOneBy =
      mockDepthDescriptionRepository.findOneBy || jest.fn();
    mockDepthDescriptionRepository.findOne =
      mockDepthDescriptionRepository.findOne || jest.fn();
