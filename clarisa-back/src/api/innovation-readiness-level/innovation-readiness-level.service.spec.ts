...
describe('InnovationReadinessLevelService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: InnovationReadinessLevelRepository,
          useValue: mockInnovationReadinessLevelRepository,
        },
      ],
    }).compile();
  ...
  it('should return items on findAll with SHOW_ALL', async () => {
    ...
    Object.keys(mockInnovationReadinessLevelRepository).forEach((k) => {
      if (
        typeof mockInnovationReadinessLevelRepository[k]?.mockResolvedValue ===
        'function'
      ) {
  ...
  it('should return a single item on findOne', async () => {
    const mockItem = { id: 1 };
    mockInnovationReadinessLevelRepository.findOneBy =
      mockInnovationReadinessLevelRepository.findOneBy || jest.fn();
    mockInnovationReadinessLevelRepository.findOne =
      mockInnovationReadinessLevelRepository.findOne || jest.fn();
    mockInnovationReadinessLevelRepository.findOneBy.mockResolvedValue(
      mockItem,
    );
  ...
  it('should save items on update', async () => {
    ...
    expect(mockInnovationReadinessLevelRepository.save).toHaveBeenCalledWith(
      dto,
    );
  });
});
