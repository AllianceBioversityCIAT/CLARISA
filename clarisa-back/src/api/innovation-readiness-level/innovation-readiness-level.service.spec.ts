...
describe('InnovationReadinessLevelService', () => {
  ...
  const mockInnovationReadinessLevelRepository: any = {
    find: jest.fn(),
    findOneBy: jest.fn(),
    save: jest.fn(),
    findOne: jest.fn(),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: InnovationReadinessLevelRepository, useValue: mockInnovationReadinessLevelRepository },
      ],
    }).compile();
  ...
  it('should be defined', () => {
    ...
    it('should return items on findAll with SHOW_ALL', async () => {
      ...
      Object.keys(mockInnovationReadinessLevelRepository).forEach(k => {
        if (typeof mockInnovationReadinessLevelRepository[k]?.mockResolvedValue === 'function') {
          mockInnovationReadinessLevelRepository[k].mockResolvedValue(mockItems);
        }
      });
    ...
    it('should return a single item on findOne', async () => {
      const mockItem = { id: 1 };
      mockInnovationReadinessLevelRepository.findOneBy = mockInnovationReadinessLevelRepository.findOneBy || jest.fn();
      mockInnovationReadinessLevelRepository.findOne = mockInnovationReadinessLevelRepository.findOne || jest.fn();
      mockInnovationReadinessLevelRepository.findOneBy.mockResolvedValue(mockItem);
      mockInnovationReadinessLevelRepository.findOne.mockResolvedValue(mockItem);
    ...
    it('should save items on update', async () => {
      const dto = [{ id: 1 }];
      mockInnovationReadinessLevelRepository.save.mockResolvedValue(dto);

      const result = await service.update(dto as any);
      expect(mockInnovationReadinessLevelRepository.save).toHaveBeenCalledWith(dto);
    });
});
