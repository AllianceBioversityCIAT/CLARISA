  ...
  it('should be defined', () => {
    ...
    it('should return items on findAll with SHOW_ALL', async () => {
      ...
      Object.keys(mockInstitutionRepository).forEach(k => {
        if (typeof mockInstitutionRepository[k]?.mockResolvedValue === 'function') {
    ...
    it('should return a single item on findOne', async () => {
      const mockItem = { id: 1 };
      mockInstitutionRepository.findOneBy = mockInstitutionRepository.findOneBy || jest.fn();
      mockInstitutionRepository.findOne = mockInstitutionRepository.findOne || jest.fn();
