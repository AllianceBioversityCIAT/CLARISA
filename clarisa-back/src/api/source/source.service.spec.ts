...
describe('SourceService', () => {
  ...
  it('should be defined', () => {
    ...
    it('should return items on findAll with SHOW_ALL', async () => {
      ...
      Object.keys(mockSourceRepository).forEach(k => {
        if (typeof mockSourceRepository[k]?.mockResolvedValue === 'function') {
          mockSourceRepository[k].mockResolvedValue(mockItems);
        }
      });
    ...
    it('should return a single item on findOne', async () => {
      const mockItem = { id: 1 };
      mockSourceRepository.findOneBy = mockSourceRepository.findOneBy || jest.fn();
      mockSourceRepository.findOne = mockSourceRepository.findOne || jest.fn();
      mockSourceRepository.findOneBy.mockResolvedValue(mockItem);
      mockSourceRepository.findOne.mockResolvedValue(mockItem);
