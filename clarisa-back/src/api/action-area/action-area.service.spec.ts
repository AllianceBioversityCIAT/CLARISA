...
describe('ActionAreaService', () => {
  ...
  const mockActionAreaRepository: any = {
    find: jest.fn(),
    findOneBy: jest.fn(),
    save: jest.fn(),
    findOne: jest.fn(),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ActionAreaService,
        { provide: ActionAreaRepository, useValue: mockActionAreaRepository },
      ],
    }).compile();
  ...
  it('should be defined', () => {
    ...
    it('should return items on findAll with SHOW_ALL', async () => {
      ...
      Object.keys(mockActionAreaRepository).forEach(k => {
        if (typeof mockActionAreaRepository[k]?.mockResolvedValue === 'function') {
          mockActionAreaRepository[k].mockResolvedValue(mockItems);
        }
      });
    ...
    it('should return a single item on findOne', async () => {
      const mockItem = { id: 1 };
      mockActionAreaRepository.findOneBy = mockActionAreaRepository.findOneBy || jest.fn();
      mockActionAreaRepository.findOne = mockActionAreaRepository.findOne || jest.fn();
      mockActionAreaRepository.findOneBy.mockResolvedValue(mockItem);
      mockActionAreaRepository.findOne.mockResolvedValue(mockItem);
    ...
    it('should save items on update', async () => {
      const dto = [{ id: 1 }];
      mockActionAreaRepository.save.mockResolvedValue(dto);

      const result = await service.update(dto as any);
      expect(mockActionAreaRepository.save).toHaveBeenCalledWith(dto);
    });
});
