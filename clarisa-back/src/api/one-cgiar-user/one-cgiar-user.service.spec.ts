...
describe('OneCgiarUserService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        OneCgiarUserService,
        { provide: OneCgiarUserRepository, useValue: mockOneCgiarUserRepository },
      ],
    }).compile();
  ...
  it('should be defined', () => {
    ...
    it('should return items on findAll with SHOW_ALL', async () => {
      ...
      Object.keys(mockOneCgiarUserRepository).forEach(k => {
        if (typeof mockOneCgiarUserRepository[k]?.mockResolvedValue === 'function') {
    ...
    it('should return a single item on findOne', async () => {
      const mockItem = { id: 1 };
      mockOneCgiarUserRepository.findOneBy = mockOneCgiarUserRepository.findOneBy || jest.fn();
      mockOneCgiarUserRepository.findOne = mockOneCgiarUserRepository.findOne || jest.fn();
