...
describe('EnvironmentalBenefitService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: EnvironmentalBenefitRepository,
          useValue: mockEnvironmentalBenefitRepository,
        },
      ],
    }).compile();
  ...
  it('should return items on findAll with SHOW_ALL', async () => {
    ...
    Object.keys(mockEnvironmentalBenefitRepository).forEach((k) => {
      if (
        typeof mockEnvironmentalBenefitRepository[k]?.mockResolvedValue ===
        'function'
      ) {
  ...
  it('should return a single item on findOne', async () => {
    const mockItem = { id: 1 };
    mockEnvironmentalBenefitRepository.findOneBy =
      mockEnvironmentalBenefitRepository.findOneBy || jest.fn();
    mockEnvironmentalBenefitRepository.findOne =
      mockEnvironmentalBenefitRepository.findOne || jest.fn();
