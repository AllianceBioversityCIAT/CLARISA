  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: GeneralAcronymRepository, useValue: mockGeneralAcronymRepository },
      ],
    }).compile();
  ...
  it('should be defined', () => {
    ...
    it('should return items on findAll with SHOW_ALL', async () => {
      ...
      Object.keys(mockGeneralAcronymRepository).forEach(k => {
        if (typeof mockGeneralAcronymRepository[k]?.mockResolvedValue === 'function') {
    ...
    it('should return a single item on findOne', async () => {
      const mockItem = { id: 1 };
      mockGeneralAcronymRepository.findOneBy = mockGeneralAcronymRepository.findOneBy || jest.fn();
      mockGeneralAcronymRepository.findOne = mockGeneralAcronymRepository.findOne || jest.fn();
