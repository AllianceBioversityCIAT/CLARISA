...
describe('RegionTypeService', () => {
  ...
  it('should return items on findAll with SHOW_ALL', async () => {
    ...
    Object.keys(mockRegionTypeRepository).forEach((k) => {
      if (
        typeof mockRegionTypeRepository[k]?.mockResolvedValue === 'function'
      ) {
  ...
  it('should return a single item on findOne', async () => {
    const mockItem = { id: 1 };
    mockRegionTypeRepository.findOneBy =
      mockRegionTypeRepository.findOneBy || jest.fn();
    mockRegionTypeRepository.findOne =
      mockRegionTypeRepository.findOne || jest.fn();
