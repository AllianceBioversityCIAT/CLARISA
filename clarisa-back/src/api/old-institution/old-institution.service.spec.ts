...
describe('OldInstitutionService', () => {
  ...
  const mockOldInstitutionRepository: any = {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: OldInstitutionRepository, useValue: mockOldInstitutionRepository },
      ],
    }).compile();
  ...
  it('should be defined', () => {
    ...
    it('should return items on findAll with SHOW_ALL', async () => {
      ...
      Object.keys(mockOldInstitutionRepository).forEach(k => {
        if (typeof mockOldInstitutionRepository[k]?.mockResolvedValue === 'function') {
          mockOldInstitutionRepository[k].mockResolvedValue(mockItems);
        }
      });
    ...
    it('should return a single item on findOne', async () => {
      const mockItem = { id: 1 };
      mockOldInstitutionRepository.findOneBy = mockOldInstitutionRepository.findOneBy || jest.fn();
      mockOldInstitutionRepository.findOne = mockOldInstitutionRepository.findOne || jest.fn();
      mockOldInstitutionRepository.findOneBy.mockResolvedValue(mockItem);
      mockOldInstitutionRepository.findOne.mockResolvedValue(mockItem);
    ...
    it('should save items on update', async () => {
      const dto = [{ id: 1 }];
      mockOldInstitutionRepository.save.mockResolvedValue(dto);

      const result = await service.update(dto as any);
      expect(mockOldInstitutionRepository.save).toHaveBeenCalledWith(dto);
    });
});
