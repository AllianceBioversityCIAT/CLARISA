...
export class ActionAreaTocServices {
        ...
        for (let actionArea of actionAreaToc) {
          if (
            ...
            actionAreaDto.outcome_id =
              typeof actionArea.outcome_id == "number"
                ? actionArea.outcome_id
                : null;

            actionAreaDto.statement =
              typeof actionArea.statement == "string"
                ? actionArea.statement
                : null;
            ...
            listOutcomeIndicators = listOutcomeIndicators.concat(
              await this.saveActionAreaTocOutcomesIndicators(
                actionArea.outcome_indicators,
                actionArea.toc_result_id,
                existingRecordSaveOrUpdate
              )
            );
            listImpactAreaToc = listImpactAreaToc.concat(
              await this.saveImpactAreaToc(
                actionArea.impact_areas,
                actionArea.toc_result_id,
                imapctAreaToc,
                existingRecordSaveOrUpdate
              )
  ...
  async saveActionAreaTocOutcomesIndicators(
    actionAreaTocOutcome: any,
    id_toc: string,
    actionarea
  ) {
    try {
      ...
      if (this.validatorType.validatorIsArray(actionAreaTocOutcome)) {
      ...
      if (this.validatorType.validatorIsArray(impactAreaToc)) {
        for (let impactsAction of impactAreaToc) {
          if (
            ...
            relationImpactArea.toc_impact_area_results_id = impactSave.find(
