...
describe('PolicyStageService', () => {
  ...
  it('should return items on findAll with SHOW_ALL', async () => {
    ...
    Object.keys(mockPolicyStageRepository).forEach((k) => {
      if (
        typeof mockPolicyStageRepository[k]?.mockResolvedValue === 'function'
      ) {
  ...
  it('should return a single item on findOne', async () => {
    const mockItem = { id: 1 };
    mockPolicyStageRepository.findOneBy =
      mockPolicyStageRepository.findOneBy || jest.fn();
    mockPolicyStageRepository.findOne =
      mockPolicyStageRepository.findOne || jest.fn();
