...
describe('ActionAreaOutcomeIndicatorService', () => {
  ...
  const mockActionAreaOutcomeIndicatorRepository: any = {
    actionAreaOutcomeIndicatorByAll: jest.fn(),
    actionAreaOutcomeIndicatorByAllIsActive: jest.fn(),
    findOneBy: jest.fn(),
    save: jest.fn(),
    find: jest.fn(),
    findOne: jest.fn(),
  ...
  it('should be defined', () => {
    ...
    it('should return items on findAll with SHOW_ALL', async () => {
      ...
      Object.keys(mockActionAreaOutcomeIndicatorRepository).forEach(k => {
        if (typeof mockActionAreaOutcomeIndicatorRepository[k]?.mockResolvedValue === 'function') {
          mockActionAreaOutcomeIndicatorRepository[k].mockResolvedValue(mockItems);
        }
      });
    ...
    it('should return a single item on findOne', async () => {
      const mockItem = { id: 1 };
      mockActionAreaOutcomeIndicatorRepository.findOneBy = mockActionAreaOutcomeIndicatorRepository.findOneBy || jest.fn();
      mockActionAreaOutcomeIndicatorRepository.findOne = mockActionAreaOutcomeIndicatorRepository.findOne || jest.fn();
      mockActionAreaOutcomeIndicatorRepository.findOneBy.mockResolvedValue(mockItem);
      mockActionAreaOutcomeIndicatorRepository.findOne.mockResolvedValue(mockItem);
    ...
    it('should save items on update', async () => {
      const dto = [{ id: 1 }];
      mockActionAreaOutcomeIndicatorRepository.save.mockResolvedValue(dto);

      const result = await service.update(dto as any);
      expect(mockActionAreaOutcomeIndicatorRepository.save).toHaveBeenCalledWith(dto);
    });
});
