...
describe('ActionAreaService', () => {
  ...
  it('should return items on findAll with SHOW_ALL', async () => {
    ...
    Object.keys(mockActionAreaRepository).forEach((k) => {
      if (
        typeof mockActionAreaRepository[k]?.mockResolvedValue === 'function'
      ) {
  ...
  it('should return a single item on findOne', async () => {
    const mockItem = { id: 1 };
    mockActionAreaRepository.findOneBy =
      mockActionAreaRepository.findOneBy || jest.fn();
    mockActionAreaRepository.findOne =
      mockActionAreaRepository.findOne || jest.fn();
