...
describe('HomepageClarisaEndpointService', () => {
  ...
  const mockHomepageClarisaEndpointRepository: any = {
    find: jest.fn(),
    findOneBy: jest.fn(),
    save: jest.fn(),
    findOne: jest.fn(),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: HomepageClarisaEndpointRepository, useValue: mockHomepageClarisaEndpointRepository },
      ],
    }).compile();
  ...
  it('should be defined', () => {
    ...
    it('should return items on findAll with SHOW_ALL', async () => {
      ...
      Object.keys(mockHomepageClarisaEndpointRepository).forEach(k => {
        if (typeof mockHomepageClarisaEndpointRepository[k]?.mockResolvedValue === 'function') {
          mockHomepageClarisaEndpointRepository[k].mockResolvedValue(mockItems);
        }
      });
    ...
    it('should return a single item on findOne', async () => {
      const mockItem = { id: 1 };
      mockHomepageClarisaEndpointRepository.findOneBy = mockHomepageClarisaEndpointRepository.findOneBy || jest.fn();
      mockHomepageClarisaEndpointRepository.findOne = mockHomepageClarisaEndpointRepository.findOne || jest.fn();
      mockHomepageClarisaEndpointRepository.findOneBy.mockResolvedValue(mockItem);
      mockHomepageClarisaEndpointRepository.findOne.mockResolvedValue(mockItem);
    ...
    it('should save items on update', async () => {
      const dto = [{ id: 1 }];
      mockHomepageClarisaEndpointRepository.save.mockResolvedValue(dto);

      const result = await service.update(dto as any);
      expect(mockHomepageClarisaEndpointRepository.save).toHaveBeenCalledWith(dto);
    });
});
