...
describe('CountryService', () => {
  ...
  it('should be defined', () => {
    ...
    it('should return items on findAll with SHOW_ALL', async () => {
      ...
      Object.keys(mockCountryRepository).forEach(k => {
        if (typeof mockCountryRepository[k]?.mockResolvedValue === 'function') {
          mockCountryRepository[k].mockResolvedValue(mockItems);
        }
      });
    ...
    it('should return a single item on findOne', async () => {
      const mockItem = { id: 1 };
      mockCountryRepository.findOneBy = mockCountryRepository.findOneBy || jest.fn();
      mockCountryRepository.findOne = mockCountryRepository.findOne || jest.fn();
      mockCountryRepository.findOneBy.mockResolvedValue(mockItem);
      mockCountryRepository.findOne.mockResolvedValue(mockItem);
    ...
    it('should save items on update', async () => {
      const dto = [{ id: 1 }];
      mockCountryRepository.save.mockResolvedValue(dto);

      const result = await service.update(dto as any);
      expect(mockCountryRepository.save).toHaveBeenCalledWith(dto);
    });
});
