...
describe('TechnicalFieldService', () => {
  ...
  const mockTechnicalFieldRepository: any = {
    find: jest.fn(),
    findOneBy: jest.fn(),
    save: jest.fn(),
    findOne: jest.fn(),
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        TechnicalFieldService,
        { provide: TechnicalFieldRepository, useValue: mockTechnicalFieldRepository },
      ],
    }).compile();
  ...
  it('should be defined', () => {
    ...
    it('should return items on findAll with SHOW_ALL', async () => {
      ...
      Object.keys(mockTechnicalFieldRepository).forEach(k => {
        if (typeof mockTechnicalFieldRepository[k]?.mockResolvedValue === 'function') {
          mockTechnicalFieldRepository[k].mockResolvedValue(mockItems);
        }
      });
    ...
    it('should return a single item on findOne', async () => {
      const mockItem = { id: 1 };
      mockTechnicalFieldRepository.findOneBy = mockTechnicalFieldRepository.findOneBy || jest.fn();
      mockTechnicalFieldRepository.findOne = mockTechnicalFieldRepository.findOne || jest.fn();
      mockTechnicalFieldRepository.findOneBy.mockResolvedValue(mockItem);
      mockTechnicalFieldRepository.findOne.mockResolvedValue(mockItem);
    ...
    it('should save items on update', async () => {
      const dto = [{ id: 1 }];
      mockTechnicalFieldRepository.save.mockResolvedValue(dto);

      const result = await service.update(dto as any);
      expect(mockTechnicalFieldRepository.save).toHaveBeenCalledWith(dto);
    });
});
