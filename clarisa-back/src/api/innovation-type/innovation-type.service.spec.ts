  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: InnovationTypeRepository, useValue: mockInnovationTypeRepository },
      ],
    }).compile();
  ...
  it('should be defined', () => {
    ...
    it('should return items on findAll with SHOW_ALL', async () => {
      ...
      Object.keys(mockInnovationTypeRepository).forEach(k => {
        if (typeof mockInnovationTypeRepository[k]?.mockResolvedValue === 'function') {
    ...
    it('should return a single item on findOne', async () => {
      const mockItem = { id: 1 };
      mockInnovationTypeRepository.findOneBy = mockInnovationTypeRepository.findOneBy || jest.fn();
      mockInnovationTypeRepository.findOne = mockInnovationTypeRepository.findOne || jest.fn();
